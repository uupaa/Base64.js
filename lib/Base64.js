 ...
 */
    if ("btoa" in global) {
        ...
        return global["btoa"]( TypedArrayToString( StringToUint8Array(source) ) ); // low-pass filter
    }
    return TypedArrayToString( Base64_encode( StringToUint8Array(source) ) );
}
 ...
 */
    ...
    return TypedArrayToString( Base64_decode( StringToUint8Array(source) ) );
}
...
//}@dev
    ...
    if (_runOnNode) {
        return StringToUint8Array( new Buffer(source).toString("base64") );
    }
...
//}@dev
    ...
    if (_runOnNode) {
        return new Uint8Array( new Buffer(TypedArrayToString(source), "base64") );
    }
...
function StringToUint8Array(source) { // @arg BinaryString
                                      // @ret Uint8Array
...
//function $some(val, str) { return global["Valid"] ? global["Valid"].some(val, str) : true; }
